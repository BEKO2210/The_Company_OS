...
import type {
  Agent, Approval, AuditLogEntry, Department,
  FinanceEntry, Incident, ProductStudio, Risk, Workflow,
  DailyReport, WeeklySummary,
  ReportEvent, AgentStatusChange, FinanceUpdate,
  AgentPerformance,
  RiskDevelopment,
  UpcomingDeadline,
  WorkflowStep,
...
export function generateWeeklySummary(
  ...
  const agentPerformance: AgentPerformance[] = allAgents
    ...
    .map((agent) => {
      const tasksCompleted =
        agent.kpis?.reduce((sum: number, kpi: { value: string }) => {
          const val = parseInt(kpi.value) || 0;
          return sum + val;
        }, 0) || 0;

      const kpiEfficiency =
        agent.kpis?.map((kpi: { value: string; target: string }) => {
      ...
      const efficiency =
        kpiEfficiency.length > 0
          ? Math.round(
              kpiEfficiency.reduce((s: number, v: number) => s + v, 0) / kpiEfficiency.length
            )
          : 0;
      ...
      return {
        ...
        efficiency,
        trend: (efficiency >= 90 ? 'up' : efficiency >= 70 ? 'stable' : 'down') as 'up' | 'down' | 'stable',
      };
    })
  ...
  const upcomingDeadlines: UpcomingDeadline[] = [
    ...
    ...allWorkflows
      .filter((w) => w.steps.some((s: WorkflowStep) => s.status === 'in-progress'))
      .map((w) => {
        const progress =
          w.steps.filter((s: WorkflowStep) => s.status === 'completed').length /
          w.steps.length;
